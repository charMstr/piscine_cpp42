...
/*   Updated: 2020/12/16 14:54:21 by charmstr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Form::beSigned(Bureaucrat const &who)
{
	...
	{
		std::stringstream ss(who.getName(), std::ios_base::app | std::ios_base::out);
		ss << " cant sign "<< getName() \
			<< " because Bureaucrat::GradeTooLowException" << std::endl;	
...
Form::execute(Bureaucrat const &executor) const
{
	...
	if (!getSigned())
	{
		std::stringstream ss(executor.getName(), std::ios_base::app | std::ios_base::out);
		ss << " cant execute "<< getName() \
			<< " because Form::NotSignedYetException" << std::endl;	
		throw MyException(ss.str());
	}
